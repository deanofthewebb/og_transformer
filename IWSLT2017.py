...
def IWSLT2017(root='.data', split=('train', 'valid', 'test'), language_pair=('de', 'en')):
    ...
    **Language pairs**:
    ...
    iwslt_tar = os.path.join(root, SUPPORTED_DATASETS['_PATH'].split(".")[0], 'texts/DeEnItNlRo/DeEnItNlRo', 'DeEnItNlRo-DeEnItNlRo/')
    ...
    extracted_files = glob.glob(iwslt_tar)
    file_archives = []
    for fname in extracted_files:
        if 'xml' not in fname and "tags" not in fname:
